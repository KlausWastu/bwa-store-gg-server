...
module.exports = {
    landingPage : async (req,res) => {
        try {
            ...
            .select('_id name status category thumbnail nominals')
    ...
    checkout : async (req,res) => {
        try {
            ...
            .populate('category') 
            .populate('user') // Butuh phoneNumber
            if(!res_voucher) return res.status(404).json({message: 'Voucher game tidak ditemukan'})

            // const  res_player = await Player .findOne({_id : username})
            // if(!res_player) return res.status(404).json({message: 'Player tidak ditemukan'})
    ...
    history : async(req,res)=> {
        try {
            const {status = ''} = req.query //query itu sama dengan req.params
            let criteria = {}
            if(status.length){
                criteria ={ 
                    ...criteria,
            ...
            res.status(201).json({data:history, total: total.length ? total[0].value : null})
    ...
    historyDetail : async(req,res) => {
        try {
            ...
            if(!history) return res.status(404).json({
                message : `History dari ${id} tidak ditemukan`
            })
